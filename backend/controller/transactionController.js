...
export const getDashboarInformation = async (req, res) => {
  try {
    const { userId } = req.body.user;
    ...
    const transactionResult = await pool.query({
      text: `
        SELECT type, SUM(amount) AS totalAmount FROM tbltransaction 
        WHERE user_id = $1 
        GROUP BY type
      `,
      values: [userId],
    ...
    const lastAccount = lastAccountsResult.rows;
...
export const addTransation = async (req, res) => {
  try {
    ...
    //CREATE TRANSACTION IN TABLE tbltransaction
    ...
    //added funds to new account
    await pool.query({
      text: `
      INSERT INTO tbltransaction(user_id, description, type, status, amount, source)
      VALUES ($1, $2, $3, $4, $5, $6)
      `,
      values: [userId, description, "expense", "complete", amount, source],
    });

    //Finalize the transaction if everything is ok
...
export const transferMoneyToAccount = async (req, res) => {
  try {
    ...
    await pool.query({
      text: `
        UPDATE tblaccount SET account_balance = account_balance - $1, updateat = CURRENT_TIMESTAMP 
        WHERE id = $2
      `,
